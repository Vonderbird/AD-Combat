...
public class UnitManager : MonoBehaviour
{
    ...
    public Transform yamatanBase; // Reference to the Yamatan base in the battleground
    ...
    void Start()
    {
        ...
        // Find the GameManager to use for initializing the unit's components
    ...
    public void AddUnit(GameObject unit, Vector3 originalPosition)
    {
        ...
        // Initialize the unit using the GameManager or other relevant systems
        InitializeUnit(unit);

        Debug.Log($"Unit {unit.name} added at {originalPosition}");
    }

    private void InitializeUnit(GameObject unit)
    {
        // Example initialization steps (adjust based on your game's specific needs)
        IEntity entity = unit.GetComponent<IEntity>();
        if (entity != null)
        {
            // Add to relevant managers (e.g., unit manager, faction manager)
            gameManager.AddUnitToFaction(entity, entity.FactionID);
        }

        // Additional initialization like setting up health, movement, and attack components
        var healthComponent = unit.GetComponent<IHealthComponent>();
        if (healthComponent != null)
        {
            healthComponent.InitializeHealth(); // Hypothetical method
        }

        var movementComponent = unit.GetComponent<IMovementComponent>();
        if (movementComponent != null)
        {
            movementComponent.InitializeMovement(); // Hypothetical method
        }

        var attackComponent = unit.GetComponent<IAttackComponent>();
        if (attackComponent != null)
        {
            attackComponent.InitializeAttack(); // Hypothetical method
        }
    }
}
