...
namespace RTSEngine.EditorOnly
{
    ...
    public class EntityItemSource<T> : IItemSource where T : IEntity 
    {
        ...
        public T Source { get; }
        public GameObject Obj => Source.gameObject;
    ...
    public class RTSEngineEditor : EditorWindow
    {
        ...
        // Temp Start
            ...
            switch(MenuType)
            {
                ...
                case NavigationMenuType.ResourceBuildings:
                    ...
                    if(RTSEditorPropertiesHandler.IsPropertyEnabled(RTSEditorProperties.advancedBuildingPlacement))
                    {
                        Toggle advancedPlacerToggle = new Toggle("Advanced Building Placer? ");
                        advancedPlacerToggle.RegisterValueChangedCallback(OnAdvancedPlacerToggleValueUpdated);
                        elementPropsBox.Add(advancedPlacerToggle);
                    }
        ...
        private void OnAdvancedPlacerToggleValueUpdated(ChangeEvent<bool> evt)
        {
            if(evt.newValue)
            {
                DestroyImmediate(ItemSource.Obj.GetComponentInChildren<BuildingPlacer>());
            }
        }
        // Temp End
